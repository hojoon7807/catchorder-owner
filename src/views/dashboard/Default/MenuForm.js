...
const MenuForm = () => {
    ...
    const onSubmit = async (e) => {
        ...
        const menuObj = {
            menuCategoryIdx: sendCategoryIdx,
            name: menuName,
            price: menuPrice,
            content: menuContent,
            tag: sendTag,
        ...
        const jwtToken = getCookies('jwtToken');
        try {
            const response = await axios.post('http://52.78.243.176/web/menu', menuObj, {
                headers: {
                    'x-access-token': jwtToken
                }
            });
    ...
    return (
        <MainCard title="메뉴 추가">
            <Container maxWidth="xs">
                <Typography variant="body2">메뉴 추가 페이지</Typography>
